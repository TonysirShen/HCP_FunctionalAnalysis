...
%% roi responses - LOTC
searchNames = {'mmpLLOTC','mmpRLOTC'}
contrast = {'FacesVsAllOthers','BodyVsAllOthers','ToolsVsAllOthers','PlacesVsAllOthers'}
for s=1:length(searchNames)
    for c = 1:length(contrast)
    disp(['Start extracting for   ' searchNames{s} '  ' contrast{c}])
    hcpSm4ROIExtract_Analysis('workingmemory',contrast{c},searchNames{s},1);
    end
end 
...
for s=1:length(searchNames)
    ...
    task = contrast_names{t};
    disp(['Start extracting for   ' searchName '  ' task]);
    hcpROIExtractBySize('workingmemory',task,searchName,1,'outputDir','group');%'invertStats',1
    end
end
...
%% ROIbySize - PR-Tools
contrast_names = {'ToolsVsAllOthers'};
searchNames = {'handDrawnLPRCThrP6','handDrawnRPRCThrP6'};
for s=1:length(searchNames) 
    searchName = searchNames{s};
    for t = 1:length(contrast_names)
    task = contrast_names{t};
    disp(['Start extracting for   ' searchName '  ' task]);
    hcpROIExtractBySize('workingmemory',task,searchName,1,'outputDir','ReplicationResults');%'invertStats',1
    end
end
