...
function hcpROIExtract(defineROITask,contrastName,searchName,isCifti,varargin)

% Define constants
studyDir = '/mnt/local_share/HCP';
% studyDir = '/Volumes/human_neuroimaging_research/HCP';
fppDir = [studyDir '/derivatives/fpp'];
allSubj = readtable([studyDir '/scripts/hcpParticipantIds.xlsx']);
subjects = cell(allSubj{:,'Participant_ID'});
...
%     maskPath = [funcDir '/' subject '_space-' statSpace '_desc-brainNonZero_mask' imageExt];
    if isCifti
        ...
        subjStr = [subject '_'];
        endsee
    searchPath = [groupDir '/' subjStr 'space-' searchSpace '_desc-' searchName '_mask' imageExt];
    end
    ...
    for t=1:length(tasks)
        ...
        disp(['Extracted data for ' subject '_task-' tasks{t}]);
    end
end
